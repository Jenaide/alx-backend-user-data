...
class Auth:
    ...
    def get_reset_password_token(self, email: str) -> str:
        """
        a method that generates a reset token password for the user
        """
        try:
            user = self._db.find_user_by(email=email)
            ...
            if user:
                ...
                self._db.update_user(user.id, resetToken=resetToken)
                return resetToken
            else:
                raise ValueError()
        except NoResultFound:
    ...
    def update_password(self, reset_token: str, password: str) -> None:
        """
        a method to update a user's password if the user resets its token
        """
        user = None
        try:
            ...
            if user:
                ...
                self._db.update_user(user.id, hashed_pwd=hashed_pwd, reset_token=None)
            else:
                raise ValueError()
        except NoResultFound:
            return None
